...
This project is a **Drug–Target Interaction (DTI) recommendation system** developed for the **Big Data Management** course. It combines **Apache Spark**, **MongoDB**, and a **Streamlit analytics dashboard** to predict possible drug–target interactions and present them through an interactive bioinformatics explorer.
...
- a **Dataset Analytics** tab showing pipeline and source-level visual summaries.
...
- Build an interactive **Streamlit** interface for exploration and presentation.

## System Workflow

### 1. Data integration
Multiple DTI-related datasets are collected and merged into a common structure. In the current dashboard, the integrated pipeline is described as combining **6 DTI datasets** with p-scale normalization across **IC50 / Kd / Ki / EC50** measures.

### 2. Data preprocessing
Raw affinity values are transformed into a comparable **p-scale affinity score**, then normalized for use in the recommendation model. The dashboard also reflects this transformation visually in the dataset analytics section.

### 3. Storage in MongoDB
Processed targets, interaction records, predictions, and network-related information are stored in **MongoDB**, which acts as the serving database for the dashboard and model outputs.

### 4. Model training with Spark ALS
The project uses **Apache Spark MLlib ALS** to learn latent factors for drugs and targets and generate ranked recommendations. The current dashboard reports **RMSE = 0.2162** and **AUPR = 0.9985** for the model shown in the analytics view.

### 5. Interactive exploration
A **Streamlit-based web application** lets the user choose a target and inspect predictions, local interaction networks, compound chemistry, disease associations, and dataset-level graphs in one place.

## Current Dashboard Features

The current application interface includes the following major sections:

### Recommendations
Shows the top predicted drugs for a selected target, together with prediction scores and export support.

### Interaction Network
Visualizes:
- the selected target,
- predicted drugs from the ALS recommender,
- STRING protein–protein interaction neighbors when available.

### Chemistry
Displays the lead compound structure and molecular properties when a valid structure can be resolved from the candidate identifiers.

### Disease Context
Uses **DisGeNET** to enrich the selected target with human gene–disease association context, if the selected target can be mapped appropriately.
...
- score distribution for the selected target.

## Example Metrics

From the currently uploaded analytics view, the system reports:

- **Unique targets:** 500 
- **Integrated DTI pairs:** about 1.707 million 
- **STRING edges:** 473,860 
- **Model RMSE:** 0.2162 
- **Model AUPR:** 0.9985 

These values may change depending on the exact datasets loaded and the version of the processed database used.
...
The dashboard sidebar also reminds the user how to set up the DisGeNET token.
